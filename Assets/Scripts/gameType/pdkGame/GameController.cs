...
public class GameController : MonoBehaviour
{
    ...
    //    player.AddComponent<SimpleSmartCard>();
    ...
    //    computerOneCards.cType = CharacterType.PlayerOne ;
    ...
    //    computerTwoCards.cType = CharacterType.PlayerTwo ;
    ...
    //    GameController.UpdateIntegration(CharacterType.PlayerOne);
    //    GameController.UpdateIntegration(CharacterType.PlayerTwo);
    ...
    public void DealCards()
    {
        ...
        for (int i = 0; i < 48; i++)
        {
            ...
            GameObject playerObj = GameObject.Find(currentCharacter.ToString());
        ...
        OrderController.Instance.Init(firstPlayer);

    }
    /// <summary>
    /// 获得拥有黑桃3的玩家
    /// </summary>
    public void GetSpadeThreePlace()
    { 
    ...
    //    int computerOneIntegration = GameObject.Find(CharacterType.PlayerOne .ToString()).GetComponent<HandCards>().Integration;
    //    int computerTwoIntegration = GameObject.Find(CharacterType.PlayerTwo .ToString()).GetComponent<HandCards>().Integration;
    ...
    //        {
    //            StartCoroutine(ChangeComputer(CharacterType.PlayerOne));
    //        }

    //        if (computerTwoIntegration <= 0)
    //        {
    //            StartCoroutine(ChangeComputer(CharacterType.PlayerTwo));
    //        }
    ...
    public static void AdjustCardSpritsPosition(CharacterType type)
    {
        ...
        else
        {
            ...
            CardSprite[] cs = GameObject.Find(type.ToString()).GetComponentsInChildren<CardSprite>();
            for (int i = hc.CardsCount-1; i>-1; i--)
            {
                for (int j = 0; j < cs.Length; j++)
                {
                    if (cs[j].Poker == hc[i])
                    {
                        cs[j].GoToPosition(GameObject.Find(type.ToString()), hc.CardsCount-i);
                    }
                }
            }
        }
